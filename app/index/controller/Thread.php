...
class Thread
{
    ...
    public function index()
    {
        ...
        // 查询状态为1的数据 并且每页显示10条数据
        $list = Db::name('thread')->where(['status'=> 1, 'type' => 1, 'fid' => $fid, 'delete_time' => 0 ])->order('create_dateup', 'desc')->cache(Config::get('cache.thread_expire'))->paginate(10);
        
        // 获取分页显示
        $page = $list->render();
        
        $newest = Db::name('thread')->where(['status'=> 1, 'type' => 1, 'delete_time' => 0 ])->order('create_dateup', 'desc')->page(1,10)->cache('newest',Config::get('cache.list_expire'))->select();
        $hot = Db::name('thread')->where(['status'=> 1, 'type' => 1, 'delete_time' => 0 ])->order(['views'=>'desc', 'favorites'=>'desc', 'likes'=>'desc' ])->page(1,10)->cache('hot',Config::get('cache.list_expire'))->select();
        ...
        if(empty($uid)){
            ...
            $user = Db::name('user')->where(['uid' => $uid, 'status' =>1 ])->cache(Config::get('cache.expire'))->find();
            $avatar = $user['avatar'];
            $username = $user['username'];
        }
        $forumname = Db::name('forum')->where(['status' => 1, 'fid' => $fid])->cache(Config::get('cache.expire'))->find();
